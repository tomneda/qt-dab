...
//	See what the saved values are and restore the GUI settings
  ...
  temp = rtlsdrSettings->value("autogain", "autogain_on").toString();
  agcControl->setChecked(temp == "autogain_on");

  ppm_correction->setValue(rtlsdrSettings->value("ppm_correction", 0).toInt());
  save_gainSettings = (rtlsdrSettings->value("save_gainSettings", 1).toInt() != 0);
  rtlsdrSettings->endGroup();
  ...
  rtlsdr_set_tuner_gain(device, gainControl->currentText().toInt());
  set_ppmCorrection(ppm_correction->value());

  // and attach the buttons/sliders to the actions
  connect(gainControl,    SIGNAL(activated(const QString&)), this, SLOT(set_ExternalGain(const QString&)));
  connect(agcControl,     SIGNAL(stateChanged(int)),         this, SLOT(set_autogain(int)));
  ...
  connect(biasControl,    SIGNAL(stateChanged(int)),         this, SLOT(set_biasControl(int)));
  connect(filterSelector, SIGNAL(stateChanged(int)),         this, SLOT(set_filter(int)));

  xmlDumper = nullptr;

  // and for saving/restoring the gain setting:
  connect(this, SIGNAL(new_gainIndex(int)),   gainControl, SLOT(setCurrentIndex(int)));
  connect(this, SIGNAL(new_agcSetting(bool)), agcControl,  SLOT(setChecked(bool)));
...
rtlsdrHandler::~rtlsdrHandler ()
{
  ...
  rtlsdrSettings->setValue("autogain", agcControl->isChecked() ? "1" : "0");
...
int32_t rtlsdrHandler::getSamples(TIQSmpFlt *V, int32_t size)
{
  ...
  if (filtering)
  {
    ...
    {
      V [i] = theFilter.Pass(TIQSmpFlt(mapTable [real(temp [i]) & 0xFF],
  ...
  else
  {
    for (int i = 0; i < amount; i++)
    {
      V [i] = TIQSmpFlt(mapTable [real(temp [i]) & 0xFF],
                                  mapTable [imag(temp [i]) & 0xFF]);
    }
  }
...
bool rtlsdrHandler::load_rtlFunctions()
{
  ...
  rtlsdr_get_usb_strings = (pfnrtlsdr_get_usb_strings)GETPROCADDRESS(Handle, "rtlsdr_get_usb_strings");
  ...
  rtlsdr_set_sample_rate = (pfnrtlsdr_set_sample_rate)GETPROCADDRESS(Handle, "rtlsdr_set_sample_rate");
  ...
  rtlsdr_get_sample_rate = (pfnrtlsdr_get_sample_rate)GETPROCADDRESS(Handle, "rtlsdr_get_sample_rate");
  ...
  rtlsdr_get_tuner_gains = (pfnrtlsdr_get_tuner_gains)GETPROCADDRESS(Handle, "rtlsdr_get_tuner_gains");
  ...
  rtlsdr_set_tuner_gain_mode = (pfnrtlsdr_set_tuner_gain_mode)GETPROCADDRESS(Handle, "rtlsdr_set_tuner_gain_mode");
  ...
  rtlsdr_set_agc_mode = (pfnrtlsdr_set_agc_mode)GETPROCADDRESS(Handle, "rtlsdr_set_agc_mode");
  ...
  rtlsdr_set_center_freq = (pfnrtlsdr_set_center_freq)GETPROCADDRESS(Handle, "rtlsdr_set_center_freq");
  ...
  rtlsdr_set_tuner_bandwidth = (pfnrtlsdr_set_center_freq) GETPROCADDRESS(Handle, "rtlsdr_set_tuner_bandwidth");
  ...
  rtlsdr_get_center_freq = (pfnrtlsdr_get_center_freq)GETPROCADDRESS(Handle, "rtlsdr_get_center_freq");
  ...
  rtlsdr_get_device_count = (pfnrtlsdr_get_device_count)GETPROCADDRESS(Handle, "rtlsdr_get_device_count");
  ...
  rtlsdr_set_direct_sampling = (pfnrtlsdr_set_direct_sampling)GETPROCADDRESS(Handle, "rtlsdr_set_direct_sampling");
  ...
  rtlsdr_set_freq_correction = (pfnrtlsdr_set_freq_correction)GETPROCADDRESS(Handle, "rtlsdr_set_freq_correction");
  ...
  rtlsdr_get_device_name = (pfnrtlsdr_get_device_name)GETPROCADDRESS(Handle, "rtlsdr_get_device_name");
  ...
  rtlsdr_set_bias_tee = (pfnrtlsdr_set_bias_tee)GETPROCADDRESS(Handle, "rtlsdr_set_bias_tee");
...
{
  QString fileName = QFileDialog::getSaveFileName(nullptr, tr("Save file ..."), QDir::homePath(), tr("raw (*.raw)"));
...
static inline bool isValid(const QChar & c)
{
