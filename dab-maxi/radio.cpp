...
//	configDisplay	= new QFrame (nullptr);
  ...
  bool motselectionMode = dabSettings->value("motSlides", 0).toInt() == 1;
  if (motselectionMode)
  {
...
//	timer for autostart epg service
  ...
  connect(my_history,                SIGNAL(handle_historySelect(const QString&)), this,            SLOT(handle_historySelect(const QString&)));
  connect(this,                      SIGNAL(set_newChannel(int)),                  channelSelector, SLOT(setCurrentIndex(int)));
  connect(this,                      SIGNAL(set_newPresetIndex(int)),              presetSelector,  SLOT(setCurrentIndex(int)));
  connect(configWidget.dlTextButton, SIGNAL(clicked()),                            this,            SLOT(handle_dlTextButton()));
  connect(configWidget.loggerButton, SIGNAL(stateChanged(int)),                    this,            SLOT(handle_LoggerButton(int)));
  ...
  techData.stereoLabel->setStyleSheet("QLabel {background-color : red}");
  ...
  techData.motAvailable->setStyleSheet("QLabel {background-color : red}");
  ...
  connect(streamoutSelector,         SIGNAL(activated(int)),     this, SLOT(set_streamSelector(int)));

  my_presetHandler.loadPresets(presetFile, presetSelector);

  // Connect the buttons for the color_settings
  connect(contentButton,             SIGNAL(rightClicked()), this, SLOT(color_contentButton(void)));
  connect(detailButton,              SIGNAL(rightClicked()), this, SLOT(color_detailButton()));
  connect(resetButton,               SIGNAL(rightClicked()), this, SLOT(color_resetButton(void)));
  connect(scanButton,                SIGNAL(rightClicked()), this, SLOT(color_scanButton(void)));
  connect(show_tiiButton,            SIGNAL(rightClicked()), this, SLOT(color_tiiButton(void)));
  connect(show_correlationButton,    SIGNAL(rightClicked()), this, SLOT(color_correlationButton(void)));
  connect(show_spectrumButton,       SIGNAL(rightClicked()), this, SLOT(color_spectrumButton(void)));
  connect(snrButton,                 SIGNAL(rightClicked()), this, SLOT(color_snrButton()));
  connect(devicewidgetButton,        SIGNAL(rightClicked()), this, SLOT(color_devicewidgetButton()));
  connect(historyButton,             SIGNAL(rightClicked()), this, SLOT(color_historyButton()));
  connect(dumpButton,                SIGNAL(rightClicked()), this, SLOT(color_sourcedumpButton(void)));
  connect(configButton,              SIGNAL(rightClicked()), this, SLOT(color_configButton(void)));
  connect(configWidget.dlTextButton, SIGNAL(rightClicked()), this, SLOT(color_dlTextButton(void)));
  connect(hideButton,                SIGNAL(rightClicked()), this, SLOT(color_hideButton(void)));
  connect(scheduleButton,            SIGNAL(rightClicked()), this, SLOT(color_scheduleButton()));
  connect(prevChannelButton,         SIGNAL(rightClicked()), this, SLOT(color_prevChannelButton(void)));
  connect(nextChannelButton,         SIGNAL(rightClicked()), this, SLOT(color_nextChannelButton(void)));
  ...
  connect(techData.framedumpButton,  SIGNAL(rightClicked()), this, SLOT(color_framedumpButton(void)));
  connect(techData.audiodumpButton,  SIGNAL(rightClicked()), this, SLOT(color_audiodumpButton(void)));
  connect(techData.timeTable_button, SIGNAL(clicked()),      this, SLOT(handle_timeTable()));
  connect(muteButton,                SIGNAL(rightClicked()), this, SLOT(color_muteButton()));
  //connect(techData.muteButton,       SIGNAL(rightClicked()), this, SLOT(color_muteButton(void)));

  //display the version
  copyrightLabel->setToolTip(footText());

  connect(configWidget.set_coordinatesButton, SIGNAL(clicked()), this, SLOT(handle_set_coordinatesButton()));
...
//	seconds running and handle - if available - the tii data
  ...
  connect(&displayTimer, SIGNAL(timeout(void)), this, SLOT(updateTimeDisplay(void)));
...
//	timer for scanning
  ...
  connect(&channelTimer, SIGNAL(timeout()), this, SLOT(channel_timeOut()));

//	presetTimer
  presetTimer.setSingleShot(true);

  connect(&presetTimer, SIGNAL(timeout()), this, SLOT(setPresetStation()));
...
//	add devices to the list
  ...
  show_hide_controls(dabSettings->value("hidden", 0).toInt() == 0);

  connect(hideButton, SIGNAL(clicked()), this, SLOT(handle_hideButton()));
...
//	we wait until one is selected
  ...
  connect(deviceSelector, SIGNAL(activated(const QString&)), this, SLOT(doStart(const QString&)));
...
//	do not want here
  connect(presetSelector,  SIGNAL(activated(const QString&)), this, SLOT(handle_presetSelector(const QString&)));
  connect(channelSelector, SIGNAL(activated(const QString&)), this, SLOT(selectChannel(const QString&)));
...
//	of "reset"
  disconnect(deviceSelector, SIGNAL(activated(const QString&)), this, SLOT(doStart(const QString&)));
  disconnect(deviceSelector, SIGNAL(activated(const QString&)), this, SLOT(newDevice(const QString&)));
  connect(deviceSelector,    SIGNAL(activated(const QString&)), this, SLOT(newDevice(const QString&)));
...
//	secondariesVector. resize (0);
  ...
  connect(configWidget.tii_detectorMode, SIGNAL(stateChanged(int)), this, SLOT(handle_tii_detectorMode(int)));
...
void RadioInterface::dumpControlState(QSettings *s)
{
  ...
  s->setValue("soundchannel", streamoutSelector->currentText());

  if (inputDevice != nullptr)
  {
    s->setValue("deviceVisible", inputDevice->isHidden() ? 0 : 1);
  }

  s->setValue("spectrumVisible", my_spectrumViewer.isHidden() ? 0 : 1);
  s->setValue("tiiVisible", my_tiiViewer.isHidden() ? 0 : 1);
  s->setValue("correlationVisible", my_correlationViewer.isHidden() ? 0 : 1);
  s->setValue("snrVisible", my_snrViewer.isHidden() ? 0 : 1);
  s->setValue("utcSelector", configWidget.utcSelector->isChecked() ? 1 : 0);
  s->sync();
}
//
///////////////////////////////////////////////////////////////////////////////
...
void RadioInterface::handle_contentButton()
{
  ...
  connect(my_contentTable, SIGNAL(goService(const QString&)), this, SLOT(handle_contentSelector(const QString&)));
...
void RadioInterface::updateTimeDisplay()
{
  ...
  QString text; text.sprintf("Runtime: %02d:%02d", numberHours, numberMinutes);
...
static const char * const monthTable[] =
{
...
void RadioInterface::show_tii(int mode, int mainId, int subId)
{
  QString a = QString("TTI");
  bool found     = false;
  QString country   = "";
  bool tiiChange = false;
  bool tiiDataOk = false;

  switch (mode)
  {
  case TII_Detector::STiiInfo::EResult::NONE:       a += "(---)"; break;
  case TII_Detector::STiiInfo::EResult::NO_PK_FND:  a += "(NPF)"; break;
  case TII_Detector::STiiInfo::EResult::PK_FND_ML:  a += "(ML)";  tiiDataOk = true; break;
  case TII_Detector::STiiInfo::EResult::PK_FND_MPS: a += "(MPS)"; tiiDataOk = true; break;
  }

  a += ": " + tiiNumber(mainId) + " " + tiiNumber(subId);
  ...
  {
    if ((transmitters.at(i + 0) == (mainId & 0x7F)) &&
  ...
  if (channel.tiiFile)
  {
    if (tiiChange || (channel.transmitterName == ""))
    {
      if (!tiiProcessor.is_black(channel.Eid, mainId, subId))
      {
        ...
        else if (theName != channel.transmitterName)
        {
          channel.transmitterName = theName;
          float latitude, longitude;
          tiiProcessor.get_coordinates(&latitude, &longitude, (channel.realChannel ? channel.channelName : "any"), theName);
          fprintf(stderr, "%s (%f, %f)\n", theName.toUtf8().data(), latitude, longitude);
          ...
          float ownLatitude = dabSettings->value("latitude", 0).toFloat();
          float ownLongitude = dabSettings->value("longitude", 0).toFloat();

          if ((ownLatitude != 0) && (ownLongitude != 0))
          {
            int distance = tiiProcessor.distance(latitude, longitude, ownLatitude, ownLongitude);
            int hoek = tiiProcessor.corner(latitude, longitude, ownLatitude, ownLongitude);
            LOG("distance ", QString::number(distance));
            LOG("corner ", QString::number(hoek));
            labelText += " " + QString::number(distance) + " km" + " " + QString::number(hoek);
...
void RadioInterface::showCorrelation(int amount, int marker, QVector<int> v)
{
  if (!running.load())
  {
    return;
...
void RadioInterface::stop_sourceDumping()
{
  ...
  sf_close(rawDumper);
  rawDumper = nullptr;
  setButtonFont(dumpButton, "Dump Raw", 10);
}
...
{
  audioDumper = filenameFinder.findAudioDump_fileName(serviceLabel->text(), true);
...
{
  frameDumper = filenameFinder.findFrameDump_fileName(serviceLabel->text(), true);
...
void RadioInterface::scheduled_frameDumping(const QString &s)
{
  ...
  frameDumper = filenameFinder.findFrameDump_fileName(s, false);
...
{
  connect(contentButton,                    &verySmallPushButton::clicked,                   this, &RadioInterface::handle_contentButton);
  connect(detailButton,                     &verySmallPushButton::clicked,                   this, &RadioInterface::handle_detailButton);
  connect(resetButton,                      &verySmallPushButton::clicked,                   this, &RadioInterface::handle_resetButton);
  connect(scanButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_scanButton);
  connect(show_tiiButton,                   &verySmallPushButton::clicked,                   this, &RadioInterface::handle_tiiButton);
  connect(show_correlationButton,           &verySmallPushButton::clicked,                   this, &RadioInterface::handle_correlationButton);
  connect(show_spectrumButton,              &verySmallPushButton::clicked,                   this, &RadioInterface::handle_spectrumButton);
  connect(snrButton,                        &verySmallPushButton::clicked,                   this, &RadioInterface::handle_snrButton);
  connect(devicewidgetButton,               &verySmallPushButton::clicked,                   this, &RadioInterface::handle_devicewidgetButton);
  connect(historyButton,                    &verySmallPushButton::clicked,                   this, &RadioInterface::handle_historyButton);
  connect(dumpButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_sourcedumpButton);

  connect(nextChannelButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_nextChannelButton);
  connect(prevChannelButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_prevChannelButton);
  connect(prevServiceButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_prevServiceButton);
  connect(nextServiceButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_nextServiceButton);

  connect(techData.audiodumpButton,         &verySmallPushButton::clicked,                   this, &RadioInterface::handle_audiodumpButton);
  connect(techData.framedumpButton,         &verySmallPushButton::clicked,                   this, &RadioInterface::handle_framedumpButton);
  connect(muteButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_muteButton);
  connect(scheduleButton,                   &verySmallPushButton::clicked,                   this, &RadioInterface::handle_scheduleButton);

  connect(ensembleDisplay,                  &smallQListView::clicked,                        this, &RadioInterface::selectService);

  connect(configButton,                     &verySmallPushButton::clicked,                   this, &RadioInterface::handle_configSetting);
  connect(configWidget.muteTimeSetting,     qOverload<int>(&smallSpinBox::valueChanged),     this, &RadioInterface::handle_muteTimeSetting);
  connect(configWidget.switchDelaySetting,  qOverload<int>(&smallSpinBox::valueChanged),     this, &RadioInterface::handle_switchDelaySetting);
  connect(configWidget.orderAlfabetical,    &QRadioButton::clicked,                          this, &RadioInterface::handle_orderAlfabetical);
  connect(configWidget.orderServiceIds,     &QRadioButton::clicked,                          this, &RadioInterface::handle_orderServiceIds);
  connect(configWidget.ordersubChannelIds,  &QRadioButton::clicked,                          this, &RadioInterface::handle_ordersubChannelIds);
  connect(configWidget.scanmodeSelector,    qOverload<int>(&QComboBox::currentIndexChanged), this, &RadioInterface::handle_scanmodeSelector);
  connect(configWidget.motslideSelector,    &QCheckBox::stateChanged,                        this, &RadioInterface::handle_motslideSelector);
  connect(configWidget.saveServiceSelector, &QCheckBox::stateChanged,                        this, &RadioInterface::handle_saveServiceSelector);
  connect(configWidget.skipList_button,     &smallPushButton::clicked,                       this, &RadioInterface::handle_skipList_button);
  connect(configWidget.skipFile_button,     &smallPushButton::clicked,                       this, &RadioInterface::handle_skipFile_button);
}

void RadioInterface::disconnectGUI()
{
  disconnect(contentButton,                    &verySmallPushButton::clicked,                   this, &RadioInterface::handle_contentButton);
  disconnect(detailButton,                     &verySmallPushButton::clicked,                   this, &RadioInterface::handle_detailButton);
  disconnect(resetButton,                      &verySmallPushButton::clicked,                   this, &RadioInterface::handle_resetButton);
  disconnect(scanButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_scanButton);
  disconnect(show_tiiButton,                   &verySmallPushButton::clicked,                   this, &RadioInterface::handle_tiiButton);
  disconnect(show_correlationButton,           &verySmallPushButton::clicked,                   this, &RadioInterface::handle_correlationButton);
  disconnect(show_spectrumButton,              &verySmallPushButton::clicked,                   this, &RadioInterface::handle_spectrumButton);
  disconnect(snrButton,                        &verySmallPushButton::clicked,                   this, &RadioInterface::handle_snrButton);
  disconnect(devicewidgetButton,               &verySmallPushButton::clicked,                   this, &RadioInterface::handle_devicewidgetButton);
  disconnect(historyButton,                    &verySmallPushButton::clicked,                   this, &RadioInterface::handle_historyButton);
  disconnect(dumpButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_sourcedumpButton);
  disconnect(nextChannelButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_nextChannelButton);
  disconnect(prevChannelButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_prevChannelButton);
  disconnect(prevServiceButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_prevServiceButton);
  disconnect(nextServiceButton,                &smallPushButton::clicked,                       this, &RadioInterface::handle_nextServiceButton);

  disconnect(techData.audiodumpButton,         &verySmallPushButton::clicked,                   this, &RadioInterface::handle_audiodumpButton);
  disconnect(techData.framedumpButton,         &verySmallPushButton::clicked,                   this, &RadioInterface::handle_framedumpButton);
  disconnect(muteButton,                       &verySmallPushButton::clicked,                   this, &RadioInterface::handle_muteButton);
  disconnect(scheduleButton,                   &verySmallPushButton::clicked,                   this, &RadioInterface::handle_scheduleButton);

  disconnect(ensembleDisplay,                  &smallQListView::clicked,                        this, &RadioInterface::selectService);

  disconnect(configButton,                     &verySmallPushButton::clicked,                   this, &RadioInterface::handle_configSetting);
  disconnect(configWidget.muteTimeSetting,     qOverload<int>(&smallSpinBox::valueChanged),     this, &RadioInterface::handle_muteTimeSetting);
  disconnect(configWidget.switchDelaySetting,  qOverload<int>(&smallSpinBox::valueChanged),     this, &RadioInterface::handle_switchDelaySetting);
  disconnect(configWidget.orderAlfabetical,    &QRadioButton::clicked,                          this, &RadioInterface::handle_orderAlfabetical);
  disconnect(configWidget.orderServiceIds,     &QRadioButton::clicked,                          this, &RadioInterface::handle_orderServiceIds);
  disconnect(configWidget.ordersubChannelIds,  &QRadioButton::clicked,                          this, &RadioInterface::handle_ordersubChannelIds);
  disconnect(configWidget.scanmodeSelector,    qOverload<int>(&QComboBox::currentIndexChanged), this, &RadioInterface::handle_scanmodeSelector);
  disconnect(configWidget.motslideSelector,    qOverload<int>(&QCheckBox::stateChanged),        this, &RadioInterface::handle_motslideSelector);
  disconnect(configWidget.saveServiceSelector, &QCheckBox::stateChanged,                        this, &RadioInterface::handle_saveServiceSelector);
  disconnect(configWidget.skipList_button,     &smallPushButton::clicked,                       this, &RadioInterface::handle_skipList_button);
  disconnect(configWidget.skipFile_button,     &smallPushButton::clicked,                       this, &RadioInterface::handle_skipFile_button);
}
...
bool RadioInterface::eventFilter(QObject *obj, QEvent *event)
{
  ...
  else if ((obj == this->ensembleDisplay->viewport()) && (event->type() == QEvent::MouseButtonPress))
  {
    ...
    if (ev->buttons() & Qt::RightButton)
    {
      ...
      const int32_t rightMouseAction = 0; // TODO:

      switch(rightMouseAction)
      {
      case 0: // store right-clicked station to channel selector
      {
        audiodata ad;
      ...
      case 1: // show audio descriptor for right-clicked station
      {
        audiodata ad;
        my_dabProcessor->dataforAudioService(serviceName, &ad);
        if (ad.defined)
      ...
      case 2: // show packed descriptor for right-clicked station
      {
        packetdata pd;
...
{
  disconnect(prevServiceButton, SIGNAL(clicked()), this, SLOT(handle_prevServiceButton()));
  handle_serviceButton(BACKWARDS);
  connect(prevServiceButton, SIGNAL(clicked()), this, SLOT(handle_prevServiceButton()));
}

void RadioInterface::handle_nextServiceButton()
{
  disconnect(nextServiceButton, SIGNAL(clicked()), this, SLOT(handle_nextServiceButton()));
  handle_serviceButton(FORWARD);
  connect(nextServiceButton, SIGNAL(clicked()), this, SLOT(handle_nextServiceButton()));
}
...
{
  disconnect(my_dabProcessor, SIGNAL(No_Signal_Found()), this, SLOT(No_Signal_Found()));
...
void RadioInterface::No_Signal_Found()
{
  ...
  disconnect(my_dabProcessor, SIGNAL(No_Signal_Found(void)), this, SLOT(No_Signal_Found(void)));

  channelTimer.stop();

  disconnect(&channelTimer, SIGNAL(timeout(void)), this, SLOT(channel_timeOut(void)));
...
//	   if (scanMode != SCAN_TO_DATA)
    ...
    else     // we just continue
    {
      ...
      connect(my_dabProcessor, SIGNAL(No_Signal_Found(void)), this, SLOT(No_Signal_Found(void)));
      connect(&channelTimer,   SIGNAL(timeout(void)),         this, SLOT(channel_timeOut(void)));
...
RadioInterface::insert(std::vector<serviceId> l, serviceId n, int order)
{
...
void RadioInterface::show_hide_controls(const bool iShow)
{
  if (iShow)
  {
...
void RadioInterface::handle_muteButton()
{
  ...
  connect(&muteTimer, SIGNAL(timeout(void)), this, SLOT(muteButton_timeOut(void)));
...
void RadioInterface::set_Colors()
{
  ...
  QString framedumpButton_color    = dabSettings->value(FRAMEDUMP_BUTTON + "_color", "white").toString();
  QString framedumpButton_font     = dabSettings->value(FRAMEDUMP_BUTTON + "_font", "black").toString();
  QString audiodumpButton_color    = dabSettings->value(AUDIODUMP_BUTTON + "_color", "white").toString();
  QString audiodumpButton_font     = dabSettings->value(AUDIODUMP_BUTTON + "_font", "black").toString();
  ...
  QString hideButton_color         = dabSettings->value(HIDE_BUTTON + "_color", "black").toString();
  QString hideButton_font          = dabSettings->value(HIDE_BUTTON + "_font", "white").toString();
  ...
  contentButton->setStyleSheet(temp.arg(contentButton_color, contentButton_font));
  detailButton->setStyleSheet(temp.arg(detailButton_color, detailButton_font));
  resetButton->setStyleSheet(temp.arg(resetButton_color, resetButton_font));
  scanButton->setStyleSheet(temp.arg(scanButton_color, scanButton_font));

  show_tiiButton->setStyleSheet(temp.arg(tiiButton_color, tiiButton_font));
  show_correlationButton->setStyleSheet(temp.arg(correlationButton_color, correlationButton_font));
  show_spectrumButton->setStyleSheet(temp.arg(spectrumButton_color, spectrumButton_font));
  snrButton->setStyleSheet(temp.arg(snrButton_color, snrButton_font));
  devicewidgetButton->setStyleSheet(temp.arg(devicewidgetButton_color, devicewidgetButton_font));

  historyButton->setStyleSheet(temp.arg(historyButton_color, historyButton_font));
  dumpButton->setStyleSheet(temp.arg(dumpButton_color, dumpButton_font));
  ...
  techData.framedumpButton->setStyleSheet(temp.arg(framedumpButton_color, framedumpButton_font));
  techData.audiodumpButton->setStyleSheet(temp.arg(audiodumpButton_color, audiodumpButton_font));
  ...
  hideButton->setStyleSheet(temp.arg(hideButton_color, hideButton_font));
  scheduleButton->setStyleSheet(temp.arg(scheduleButton_color, scheduleButton_font));
}
...
{
  dabSettings->setValue("motSlides", configWidget.motslideSelector->isChecked() ? 1 : 0);
}
...
{
  const bool b = configWidget.tii_detectorMode->isChecked();
