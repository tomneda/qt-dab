...
fftHandler::fftHandler (uint8_t mode)
  : p(mode)
{
  this->fftSize = p.get_T_u();
  vector        = (TIQSmpFlt *)
                  FFTW_MALLOC(sizeof(TIQSmpFlt) * fftSize);
...
TIQSmpFlt *fftHandler::getVector()
{
  return vector;
}
...
common_ifft::common_ifft (int32_t fft_size)
{
  ...
  vector         = (TIQSmpFlt *)FFTW_MALLOC(sizeof(TIQSmpFlt) * fft_size);
...
TIQSmpFlt *common_ifft::getVector()
{
  return vector;
}
