...
public:
  ...
  void processBlock_0(TIQSmpFlt *);
  void process_Msc(TIQSmpFlt *, int);

  bool set_Channel(descriptorType *, RingBuffer<int16_t> *, RingBuffer<uint8_t> *);
...
private:
  ...
  std::vector<std::vector<TIQSmpFlt> > command;
  int16_t                                         amount;
  void run();
