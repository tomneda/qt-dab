...
void mscHandler::processBlock_0(TIQSmpFlt *b)
{
#ifdef  __MSC_THREAD__
  bufferSpace.acquire(1);
  memcpy(command [0].data(), b, params.get_T_u() * sizeof(TIQSmpFlt));
  helper.lock();
  amount++;
...
#ifdef  __MSC_THREAD__
void mscHandler::process_Msc(TIQSmpFlt *b, int blkno)
{
  bufferSpace.acquire(1);
  memcpy(command [blkno].data(), b, params.get_T_u() * sizeof(TIQSmpFlt));
  helper.lock();
  amount++;
  commandHandler.wakeOne();
  helper.unlock();
...
void mscHandler::run()
{
  ...
  while (running.load())
  {
    ...
    {
      memcpy(fft_buffer, command [currentBlock].data(), params.get_T_u() * sizeof(TIQSmpFlt));
//
...
//      we make the bits into softbits in the range -127 .. 127
      ...
      memcpy(phaseReference.data(), fft_buffer,
             params.get_T_u() * sizeof(TIQSmpFlt));
...
void mscHandler::process_Msc(TIQSmpFlt *b, int blkno)
{
  ...
  memcpy(fft_buffer, b, params.get_T_u() * sizeof(TIQSmpFlt));
  ...
  memcpy(phaseReference.data(), fft_buffer, params.get_T_u() * sizeof(TIQSmpFlt));
}
#endif
...
void mscHandler::stopService(descriptorType *d)
{
  ...
  for (uint32_t i = 0; i < theBackends.size(); i++)
  {
...
bool mscHandler::set_Channel(descriptorType *d, RingBuffer<int16_t> *audioBuffer, RingBuffer<uint8_t> *dataBuffer)
{
  ...
  theBackends.push_back(new Backend(myRadioInterface, d, audioBuffer, dataBuffer, frameBuffer));

  fprintf(stderr, "we have now %lu backends running\n", theBackends.size());
  return true;
}
