...
rawReader::rawReader(rawFiles *mr, FILE *filePointer, RingBuffer<TIQSmpFlt> *_I_Buffer)
{
  mpParent      = mr;
  mpFilePointer = filePointer;
  mp_I_Buffer   = _I_Buffer;

  fseek(filePointer, 0, SEEK_END);
  mFileLength = ftell(filePointer);
  fprintf(stderr, "fileLength = %d\n", (int)mFileLength);
  fseek(filePointer, 0, SEEK_SET);
  mPeriod = (32768 * 1000) / (2 * 2048);           // full IQś read
  fprintf(stderr, "Period = %ld\n", mPeriod);
  mpBi = new uint8_t [BUFFERSIZE];
...
void rawReader::run()
{
  ...
  int       i;
  int       teller = 0;
  TIQSmpFlt localBuffer[BUFFERSIZE / 2];

  connect(this, SIGNAL(setProgress(int,float)), mpParent, SLOT(setProgress(int,float)));

  fseek(mpFilePointer, 0, SEEK_SET);
  mRunning.store(true);
  nextStop = getMyTime();
  try {
    while (mRunning.load())
    {
      while (mp_I_Buffer->WriteSpace() < BUFFERSIZE + 10)
      {
        if (!mRunning.load())
        {
          throw (32);
      ...
      nextStop += mPeriod;
      int n = fread(mpBi, sizeof(uint8_t), BUFFERSIZE, mpFilePointer);
      if (n < BUFFERSIZE)
      {
      ...
      {
        localBuffer [i] = TIQSmpFlt(
          4 * mapTable [mpBi [2 * i]],
          4 * mapTable [mpBi [2 * i + 1]]);
      }
      mp_I_Buffer->putDataIntoBuffer(localBuffer, BUFFERSIZE / 2);
