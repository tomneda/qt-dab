  ...
  , my_fftHandler(p->dabMode)
{
  ...
  for (i = 0; i < T_u; i++)
  {
    refTable [i] = TIQSmpFlt (0, 0);
  }

  for (i = 1; i <= params.get_carriers() / 2; i++)
  {
    Phi_k              = get_Phi(i);
    refTable [i]       = TIQSmpFlt (cos(Phi_k), sin(Phi_k));
    Phi_k              = get_Phi(-i);
    refTable [T_u - i] = TIQSmpFlt (cos(Phi_k), sin(Phi_k));
  }
...
int32_t phaseReference::findIndex(std::vector<TIQSmpFlt> v, int threshold)
{
  int32_t i;
  int32_t maxIndex = -1;
  float   sum      = 0;
  ...
  memcpy(fft_buffer, v.data(), T_u * sizeof(TIQSmpFlt));
...
int16_t phaseReference::estimate_CarrierOffset(std::vector<TIQSmpFlt> v)
{
...
float phaseReference::estimate_FrequencyOffset(std::vector<TIQSmpFlt> v)
{
  int16_t i;
  float   pd = 0;
  ...
  {
    TIQSmpFlt a1 = refTable [(T_u + i) % T_u] * conj(refTable [(T_u + i + 1) % T_u]);
    TIQSmpFlt a2 = fft_buffer [(T_u + i) % T_u] * conj(fft_buffer [(T_u + i + 1) % T_u]);
    pd += arg(a2) - arg(a1);
  }
...
float phaseReference::phase(std::vector<TIQSmpFlt> v, int Ts)
{
...
#ifdef  __WITH_JAN__
void phaseReference::estimate(std::vector<TIQSmpFlt>& v)
{
  TIQSmpFlt h_td[TAPS];
