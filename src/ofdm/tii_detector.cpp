...
TII_Detector::TII_Detector(uint8_t dabMode, int16_t depth)
  : mParams(dabMode)
  , mMyFFTHandler(dabMode)
{
  mDepth    = depth;
  mT_u      = mParams.get_T_u();
  mCarriers = mParams.get_carriers();

  mTheBuffer.resize(mT_u);

  mpFFTBuffer = mMyFFTHandler.getVector();

  // create a Hamming window
  mWindow.resize(mT_u);

  for (int16_t i = 0; i < mT_u; i++)
  {
  ...
  {
    mpInvTable[i] = 0xFF;  // fix
  }

  for (int16_t i = 0; i < 70; ++i)
  {
    mpInvTable[table[i]] = i;
  }

//  for (int16_t i = 71; i < 256; i++)
//  {
//    mpInvTable[i] = -1; // this is a bug?? this init has to taken before a bit earlier (see "fix")
//  }

//  for (int16_t i = 0; i < 255; ++i)
//  {
//    if (mpInvTable[i] != 0xFF)
//    {
//      std::cout << std::hex << i << ": " << (int)mpInvTable[i] << std::endl;
//    }
//  }

  mDetectMode_new = false;
}
...
void TII_Detector::reset()
{
  ...
  {
    mTheBuffer[i] = TIQSmpFlt(0, 0);
  }
}

//	To eliminate (reduce?) noise in the input signal, we might
//	add a few spectra before computing (up to the user)
void TII_Detector::addBuffer(std::vector<TIQSmpFlt> v)
{
  for (int32_t i = 0; i < mT_u; i++)
  {
    mpFFTBuffer[i] = cmul(v[i], mWindow[i]);
  }
  
  mMyFFTHandler.do_FFT();

  for (int32_t i = 0; i < mT_u; i++)
  {
    mTheBuffer[i] += mpFFTBuffer[i];
  }
}

//	Note that the input is fft output, not yet reodered
void TII_Detector::collapse(TIQSmpFlt *inVec, float *outVec)
{
  auto get_segment_power = [&](const int32_t iSamplePairPos, const int32_t iSegmentIdx)
  {
    const int32_t carr = -mCarriers / 2 + iSegmentIdx * mCarriers / 4  + (iSegmentIdx >= 2 ? 1 /*avoid DC*/ : 0) + 2 * iSamplePairPos;
    return abs(real(inVec[(mT_u + carr) % mT_u] * conj(inVec[(mT_u + carr + 1) % mT_u])));
  };

  for (int32_t i = 0; i < mCarriers / 8; i++)                                                       // 1536 / 8 = 192
  {
    outVec[i] = get_segment_power(i, 0)  // -768+0 + 2*i
              + get_segment_power(i, 1)  // -384+0 + 2*i
              + get_segment_power(i, 2)  //    0+1 + 2*i
              + get_segment_power(i, 3); //  384+1 + 2*i
  }
}

static constexpr uint8_t bits[] = { 0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01 };

#define NUM_GROUPS    8
#define GROUPSIZE     24

TII_Detector::STiiInfo TII_Detector::processNULL()
{
  ...
  //assert(mCarriers/8 <= NUM_GROUPS * GROUPSIZE);
  collapse(mTheBuffer.data(), hulpTable);
  ...
  for (int32_t i = 0; i < NUM_GROUPS; i++) // step over (undecoded) MainIDs
  {
    for (int32_t j = 0; j < GROUPSIZE; j++) // step over SubIDs
    {
  ...
  //	te kunnen vinden
  for (int32_t i = 0; i < GROUPSIZE; i++) // step over SubIDs
  {
    for (int32_t j = 0; j < NUM_GROUPS; j++) // step over (undecoded) MainIDs
    {
  ...
  if (maxIndex < 0)
  {
    tiiInfo.set(STiiInfo::EResult::NO_PK_FND, 0, 0);
    return tiiInfo;
  }
  ...
  // we extract the four max values as bits
  {
    ...
    for (int32_t i = 0; i < 4; i++)
    {
      ...
      if (ind != -1)
      {
        x[ind] = 0; // clear found value to not be found again, so next lowest peak can be found
        pattern |= bits[ind]; // bits are reversed in value (msb<->lsb)
      }
    }

    finInd = mpInvTable[pattern]; // transform found pattern to MainID
  }

  tiiInfo.set((mDetectMode_new ? STiiInfo::EResult::PK_FND_ML : STiiInfo::EResult::PK_FND_MPS), finInd, maxIndex);
  return tiiInfo;
}
