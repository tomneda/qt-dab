...
{
  TIQSmpFlt outputBuffer[converter_16.getOutputsize()];
  float buffer[2 * converter_16.getOutputsize()];
  int32_t result;

  for (int32_t i = 0; i < amount; i++)
  {
    if (converter_16.convert(TIQSmpFlt (V[2 * i] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (int32_t j = 0; j < result; j++)
      {
...
{
  TIQSmpFlt outputBuffer[converter_24.getOutputsize()];
  float buffer[2 * converter_24.getOutputsize()];
  int32_t result;

  for (int32_t i = 0; i < amount; i++)
  {
    if (converter_24.convert(TIQSmpFlt (V[2 * i] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (int32_t j = 0; j < result; j++)
      {
        buffer [2 * j    ] = real(outputBuffer [j]);
...
{
  TIQSmpFlt outputBuffer[converter_32.getOutputsize()];
  float               buffer[2 * converter_32.getOutputsize()];
  int32_t             result;

  for (int32_t i = 0; i < amount; i++)
  {
    if (converter_32.convert(TIQSmpFlt (V[2 * i] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (int32_t j = 0; j < result; j++)
      {
        buffer [2 * j    ] = real(outputBuffer [j]);
