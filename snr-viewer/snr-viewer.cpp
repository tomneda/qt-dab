...
snrViewer::snrViewer(RadioInterface *mr, QSettings  *s)
  : myFrame(nullptr)
  , spectrum_curve("")
{
  ...
  connect(snrDumpButton,          SIGNAL(clicked()),         this, SLOT(handle_snrDumpButton()));
  connect(snrSlider,              SIGNAL(valueChanged(int)), this, SLOT(set_snrHeight(int)));
  connect(snrCompressionSelector, SIGNAL(valueChanged(int)), this, SLOT(set_snrDelay(int)));
  connect(snrLengthSelector,      SIGNAL(valueChanged(int)), this, SLOT(set_snrLength(int)));
  ...
  lpickerMachine = new QwtPickerClickPointMachine();
  ...
  plotgrid->setAxisScale(QwtPlot::yLeft, 0, plotHeight);
  plotgrid->setAxisScale(QwtPlot::xBottom, 0, plotLength - 1);
...
void snrViewer::addtoView(float v)
{
  ...
  if (displayPointer == 0)
  {
    memmove(&(Y_Buffer.data() [VIEWBUFFER_SIZE]), &(Y_Buffer.data() [0]), (plotLength - VIEWBUFFER_SIZE) * sizeof(double));
...
void snrViewer::add_snr(float sig, float noise)
{
  ...
  memmove(&(Y_Buffer.data() [1]), &(Y_Buffer.data() [0]), (plotLength - 1) * sizeof(double));
  Y_Buffer [0] = snr;
...
{
  spectrum_curve.setSamples(X_axis.data(), Y_Buffer.data(), plotLength);
  plotgrid->replot();
}
